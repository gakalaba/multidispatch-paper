...
\sys{} provides \mdl{} across shards with \stale{worst-case} latency that approaches 1/4 that of Multi-Paxos, a \sdl{} protocol, as the number of concurrent client operations increases.
...
\noindentparagraph{Components and Protocol.}
...
recently-issued operation, or null if there is none.  In addition, when
...
\noindentparagraph{Suffix-closed Failures.}
...
\noindentparagraph{\Mdl{} Ordering.}
\Mdl{} requires there exist a legal total order over all operations that respects real-time order and each client's issue order.
...
\stale{A complete proof that \sys{} provides \Mdl{} ordering is elided for brevity.}
\todo{Does our proof cover failure recovery and suffix-closed failures? If so, its weird to mention it here.}


%\subsubsection{Operation vs.\@ End-To-End Latency}
\noindentparagraph{Operation vs.\@ End-to-end Latency.}
...
than its predecessor.  \stale{Thus, while the latency of an individual operation is
