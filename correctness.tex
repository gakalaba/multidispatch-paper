...
\wl{We "case"? -> We examine and prove all possible cases}
\wl{Shard leader failure is too generic. I think it should be ``shard leader failure what an operation is committed but not ordered.}
\wl{I think the no failure case should explicitly discuss the restriction of failures to being only committed but not ordered, because before committed is... and after ordered is ...}
...
Assume shard with $e_1$ fails. \wl{cut, this is already part of the case}
...
\wl{While I think the following statement is true, I think you probably need a lemma that proves inductively that $e_2$ not failing will eventually result in getting a non-NULL value back}
