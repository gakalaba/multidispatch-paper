...
\SetKwFunction{getPredecessor}{Leader::GetPredecessor}
\SetKwFunction{getSuccessor}{Leader::GetSuccessor}
...
\begin{algorithm}[!tb]
    ...
    \Function{\Recover(\textit{E})}{ \algorithmiccomment{E := committed but not coordinated ops}\\
        \Wait receive $E_i$ from $S_i \in S$\\
        $E = \bigcup\limits_{i=1}^{n} E_{i}$\\
        \Send $\textit{getPredecessor}(\textit{e})$ to $S_i \in \mathcal{S}$ for each $e \in E$\\
        \Send $\textit{getSuccessor}(\textit{e})$ to $S_i \in \mathcal{S}$ for each $e \in E$\\
    }
    ...
    \Function{\getSuccessor(\textit{Req})}{
        \Wait receive $\textit{SubmitOpReply}(V)$ from $L_K \in \mathcal{L}$\\
        \Wait until $\textit{rcvd} = \textit{Req.seqno}$\\
        %$m.lock()$\\
        $\IfThen{\textit{prevReq} = \textit{Req}}{\textit{prevReq} \gets \textit{NULL}}$\\
