...
\begin{algorithm}[!tb]
    ...
    \Function{\Recover(\textit{E})}{
        ...
        \For{$clientgroup \in D$}{
            $clientgroup\sorting(\text{by} \enspace e.\mathrm{seqno})$\\
            \For{$e \in clientgroup$}{
                \label{B_begin}
                \Send $\textit{getPredecessorTS}(e.\mathrm{pred})$ to $S_{e.\mathrm{pred}}$\\
                \Wait receive $\textit{getPredecessorTSReply}(ts\_chain)$ from $S_e.\mathrm{pred}$\\
...
\begin{algorithm}[!tb]
    ...
    \Function{\CRReply(\textit{req}, v, \textit{timestamp\_chain})}{
