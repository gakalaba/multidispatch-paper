...
operations. Moreover, these coordination messages may overlap with the second
round of replication.

Figures~\ref{fig:sdlperf} and~\ref{fig:mdlperf} compare the performance of
Multi-Paxos with sequentially issued operations and \sys{}.  The performance
depends on the number of concurrent client requests in the execution of an
application operation as well as the number of distinct physical shards those
requests are sent to.  Let $T$ be the one-way delay between machines (clients,
leaders, and replicas).  When there are $N$ (potentially) concurrent client
requests, Multi-Paxos's latency is $4*T*N$ while \sys{}'s latency is
$6T + (N-1)*T$ in the worst case when each request lies at a different shard
