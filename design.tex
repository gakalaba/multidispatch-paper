...
Thus, we need to assume that any committed operation could have been coordinated at the \textit{old leader} and potentially coordinated its glfsuccessor.
...
Second, we must ensure failover respects suffix-closed failures.
...
\begin{algorithm}[!tb]
    ...
    \Function{\leaderHandleAccept(\text{entry})}{
        ...
        \If{$\textit{entry.committed} \land \textit{entry.coordinated}$}{
            ...
            \If{$entry.succrq \neq NULL$}{
                $\textit{SubmitCRResp}(\textit{entry.succrq}, \textit{True}, \textit{entry.ts\_chain})$ to $L_{succrq} \in \mathcal{L}$\\
            }
...
\begin{algorithm}[!tb]
    ...
    \algorithmiccomment{CR sent by client of req to shard carrying req's predecessor}\\
        ...
        \If{$\textit{entry} := \textit{orderedLog}\find(\textit{pred}) \neq \textit{NULL}$}{
            $\textit{SubmitCRResp}(\textit{req}, \textit{entry.ts\_chain})$ to $L_{req} \in \mathcal{L}$\\
            \Return\\
        }
    ...
    \Function{\CRReply(\textit{req}, \textit{timestamp\_chain})}{
        ...
        \If{$\textit{entry.committed} \land \textit{entry.coordinated}$}{
            ...
            \If{$entry.succrq \neq NULL$}{
                $\textit{SubmitCRResp}(\textit{entry.succrq}, \textit{entry.ts\_chain})$ to $L_{succrq} \in \mathcal{L}$\\
            }
            $\textit{bufferedMap}\pop(\textit{entry})$\\
            $\textit{orderedLog}\append(\textit{entry})$\\
            \Send $\textit{FinalAccept}(\textit{entry})$ to all $r \in \mathcal{R}$\\            
...
\begin{algorithm}[!tb]
    ...
    \Function{\Recover(\textit{E})}{
        ...
        \For{$\mathrm{each}$ $s \in S$}{
            ...
            \For{$\mathrm{each}$ $ts\_chain_i \in TS$}{
                \If{$ts\_chain_i \neq NULL$}{
                    $ts\_chain_i\pop()$\\
        ...
        \For{$client \in D$}{
            ...
            \For{$e \in client$}{
                $ts\_chain := []$\algorithmiccomment{Note $[] \neq NULL$}\\
                $pred\_ts \gets -1$\\
                \label{B_begin}
                \If{$\textit{e.pred} \neq \textit{NULL}$}{
                    \Send $\textit{SubmitCR}(\textit{e.pred}, \textit{e})$ to $L_{e.pred} \in \mathcal{L}$\\
                    \Wait receive $\textit{SubmitCRRespRecv}(e, ts\_chain)$ from $S_e.\mathrm{pred}$\\
                    $pred\_ts \gets ts\_chain[-1]$\\
                }
    ...
    \Function{\getChain(E)}{
        ...
        \For{$e_i \in E$}{
            \If{$\exists \enspace \textit{o} \notin \mathrm{orderedLog} \enspace \land \enspace o.\mathrm{committed} \enspace \land \enspace o.\mathrm{pred} = e$}{
                ...
                \Wait receive $\textit{getTimestampChainReply}(ts\_chain)$ from $S$ \\
                \label{mytsminus1}
                $ts\_chain\pop()$\\
                $V_i \gets ts\_chain$\\
            }
            \If{$\exists \enspace \textit{o} \in \mathrm{orderedLog} \enspace \land \enspace o.\mathrm{pred} = e$}{
                $V_i \gets$ $o.\mathrm{ts\_chain}$
            }
            \Else{
                $V_i \gets$ $NULL$
            }
...
Once all operations in the committed-but-not-ordered set are coordinated they are sorted by their timestamps (last element in timestamp chain), added to the ordered log, and replicated.
