...
\sys{} provides \mdl{} across shards with latency that approaches 1/4 that of \sdl{} as the number of concurrent client operations increases.
...
The sequential coordination ensures the legal total order across shards and client issue orders required by \mdl{} will exist.
Coordination is not replicated and requires only a single sequential shard-to-shard message for each concurrent operation, which is what enables \sys{} to provide lower latency.
...
The full client and shard protocols are specified in Algorithms~\ref{clientprotocol} and~\ref{shardprotocol}.
...
If there are none, the client sends an operation to the leader of the appropriate shard along with a \texttt{null} predecessor id.
...
In addition, when there are outstanding operations, the client sends a coordinate request message to the shard leader for the predecessor operation that indicates this operation as its \textit{successor}.
...
Then in either case it checks if it has received a coordination request for this operation, and if so immediately sends a coordination reply to the leader for the successor operation.

Once an operation is marked coordinated the leader places the operation into the next available instance in its ordered log and removes it from the committed set.
...
When a leader receives a coordinate request for an operation it has not yet coordinated it queues the request.
Conversely, when a leader receives a coordinate request for an operation it already coordinated it immediately sends the coordinate reply message to the specified successor.

\wl{NEED TO ADD WHEN ts is assigned.}
...
Ordering coordinate replies after this operation is committed ensures this operation will be executed, either by the current leader in the normal case or by a new leader in the case of failover.
Ordering coordinate replies after this operation is coordinated ensures this is transitively true for all of a client's earlier-issued concurrent operations.
...
First, an operation will fail if it times out waiting for its coordinate reply from its predecessor.
...
Second, an operation will fail if it receives a special fail\_coordinate reply instead of a coordinate reply
In both cases, once an operation is failed the shard replies to the client with the failure.
In addition, the shard sends a fail\_coordinate message to its successor operation if a coordinate request is queued.
(If no coordinate request is queued it tracks the failure so it can reply to a future coordinate request with a fail\_coordinate message.)
...
this create a cycle where $o_{a4}$ is before $o_{a1}$ on shard A, $o_{a1}$ is before $o_{b2}$ by client 1's issue order, $o_{b2}$ is before $o_{b3}$ by shard B, and $o_{b3}$ is before $o_{a4}$ by client 2's issue order.
...
Multi-Paxos with provides \sdl{} with a single round of replication for both fault tolerance and ordering.
\sys{}, in contrast, decoupling replication for fault tolerance and ordering into two rounds.
...
The coordination requests are sent by the client-library and the coordination replies are send from shard to shard.
This puts only a single shard-to-shard message in the sequential critical path of many concurrent client operations.
...
The performance depends on the number of concurrent client requests in the execution of an application operation.
...
while \sys{}'s latency is $6T + (N−1)∗T$.
Thus, while the latency of an individual operation is higher in \sys{}, an application's end-to-end latency approach $1/4$ that of Multi-Paxos as the number of concurrent client requests grows.
...
This leaves operations that are committed and potentially coordinated.
...
This bring two requirements for recovering committed-but-not-ordered operations.
...
These coordination requests serve their typical purpose and also return the predecessor\_ts.
During recovery only the predecessor\_ts is used to determine the ts of an operation.
...
Once all operations in that set have been successfully replicated and thus moved to the ordered status the new leader starts processing new operations.

This protocol avoids introducing any cycles in a way similar to the normal coordinate messages by ensuring their order is compatible with any other ordering on another shard.
Consider two operations $o_m$ and $o_n$ on this shard where there is a path in the graph of constraints between from $o_m$ to $o_n$.
...
Together, this ensures that  $\textit{ts}_\textit{m.new}$ $\le$ $\textit{ts}_\textit{m.old}$ $<$ $\ldots$ $<$ $\textit{ts}_\textit{m.old}$ and thus the recovery protocol will compatibly order $o_m$ before $o_n$ when it sorts them based on the ts.

\wl{Should we have a footnote that says ``Reasoning is similar, but significantly more complex, for cases where multiple shard leaders fail which we omit due to space constraints.''?}
