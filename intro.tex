...
\todo{make sure section 2 has an example where an update breaks the application...}
...
\sys{} provides low end-to-end latency through a careful separation of concerns that minimizes the amount of sequential coordination. % necessary between concurrent operations from a client.
...
reduces end-to-end application latency by up to 75\% in a single datacenter and up to \tbd{XX\%} in the wide-area.
\true{These largest latency improvements come at the cost of lower throughput for \sys{}.
When both systems use batching, however,
we find \sys{} achieves throughput parity with Multi-Paxos
while still providing \tbd{xx--yy\%} and \tbd{aa--bb\%} lower latency.}
...
\wl{No discussion of systems / consistency models / etc here, does that make sense? Do we need to set the stage anymore or is it sufficiently obvious its unnecessary?}
